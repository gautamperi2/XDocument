...
std::shared_ptr<AbstractXmlElement> XmlProcessing::makeDocElement(std::vector<std::shared_ptr<AbstractXmlElement>> treeFrame, std::shared_ptr<AbstractXmlElement> pRoot)
{
	std::shared_ptr<AbstractXmlElement> ptr(new DocElement(treeFrame, pRoot));
	return ptr;
}
...
DocElement::DocElement(std::vector<std::shared_ptr<AbstractXmlElement>> treeFrame, std::shared_ptr<AbstractXmlElement> pRoot)
{
	if (treeFrame.size() != 0)
	{
		for (size_t i = 0; i < treeFrame.size(); i++)
			children_.push_back(treeFrame[i]);
...
std::vector<std::shared_ptr<AbstractXmlElement>> DocElement::children() { return children_; }
